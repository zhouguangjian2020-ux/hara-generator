...
def _parse_chapter3_with_context(doc: Document) -> tuple[list[dict], dict]:
    ...
    for source_index, paragraph in enumerate(doc.paragraphs):
        ...
        if level is not None:
            ...
            explicit_number, title = split_heading_number(text)
            label = f"{explicit_number or '.'.join(label_numbers)} {title}".strip()
            node = {
                "node_id": node_id,
    ...
    summary_map = build_function_description_map(document_context, max_chars=1200)
