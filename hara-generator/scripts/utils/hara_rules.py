...
from utils.domain_packs import DomainPackSourceError, is_strict_external_pt, load_domain_pack, resolve_domain_pack_source
...
_SCENARIO_PT_SOURCE: str | None = None
...
def _load_domain_pack_scenarios(domain: str) -> dict:
    ...
    return {
        ...
        "scenario_asset": (
            str(resolve_domain_pack_source(canonical).pack_path.resolve())
            if is_strict_external_pt(canonical)
            else f"references/domain_packs/{canonical}/{canonical}.json"
        ),
    }
...
def reset_scenario_asset_caches() -> None:
    ...
    global _SCENARIO_MANIFEST_CACHE, _SCENARIO_LOOKUP_CACHE, _HARDCODED_SCEN_CACHE, _SCENARIO_PT_SOURCE
    _SCENARIO_PT_SOURCE = None
...
def load_domain_scenario(domain: str, allow_legacy: bool = True) -> dict:
    ...
    global _SCENARIO_PT_SOURCE
    canonical = canonical_scenario_domain(domain)
    if canonical == "PT":
        try:
            current_source = str(resolve_domain_pack_source(canonical).pack_path.resolve())
        except DomainPackSourceError:
            _SCENARIO_DOMAIN_CACHE.pop("PT", None)
            _SCENARIO_PT_SOURCE = None
            raise
        if _SCENARIO_PT_SOURCE != current_source:
            _SCENARIO_DOMAIN_CACHE.pop("PT", None)
            _SCENARIO_PT_SOURCE = current_source
    ...
    if isinstance(risk, dict) and risk.get("scenario_single_source_contract") and isinstance(risk.get("scenario_catalog"), dict):
        ...
        return _SCENARIO_DOMAIN_CACHE[canonical]
    if is_strict_external_pt(canonical):
        raise DomainPackSourceError("严格外部 PT 缺少场景单源合同，不允许回退旧场景资产")
...
def lookup_scenarios(domain: str, func_name: str,
    ...
    for ftype_key, ftype_data in domain_data.get("function_types", {}).items():
        ...
        return {"scenarios": fm_data["scenarios"],
                ...
                "scenario_asset": (str(resolve_domain_pack_source(canonical).pack_path.resolve())
                                   if is_strict_external_pt(canonical) else
                                   ("references/domain_packs/PT/PT.json" if canonical == "PT" else f"{canonical}.json")),
