...
def _group_domain_cases(cases: list[dict[str, Any]], selected_modes: list[str], hazard_catalog: dict[str, Any], domain: str) -> tuple[list[dict[str, Any]], str | None]:
    ...
    for key, members in ordered:
        ...
        anomaly_classes = {
            str((case.get("failure_profile") or {}).get("anomaly_class") or "").strip()
            for case in members
        }
        if len(modes) != 1 or len(anomalies) > 1 or len(anomaly_classes) != 1 or "" in anomaly_classes:
        ...
        result.append({
            ...
            "failure_profile": {
                "canonical_mode": next(iter(modes)),
            ...
            for mode in contract["selected_modes"]:
                ...
                for group in groups_by_mode.get(mode, []):
                    ...
                    mode_anomalies.append({
                        ...
                        "failure_profile": dict(group["failure_profile"]),
...
def validate_known_s3_contract(intermediate: dict[str, Any], s1: dict[str, Any], s2: dict[str, Any], s3: dict[str, Any]) -> list[str]:
    ...
    for key, expected_entry in expected.items():
        ...
        if expected_entry.get("source") == "case_library" and actual_entry.get("source") != "case_library":
            errors.append(f"[Domain Pack S3] {key[0]} / {key[1]} 的案例库 source 被修改")
        ...
        for expected_anomaly in expected_anomalies:
            ...
            for field in ("failure_profile", "case_ids", "source_refs", "source_failure_id"):
                if field in expected_anomaly and anomaly.get(field) != expected_anomaly[field]:
                    errors.append(f"[Domain Pack S3] {key[0]} / {key[1]} / {expected_anomaly['failure_id']} 的 {field} 与来源案例不一致；请重新执行 s3 generate/compose")
