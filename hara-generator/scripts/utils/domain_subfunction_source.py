...
``整车功能名称 + 子功能描述`` 的规范化精确匹配。正式运行时未知名称必须保持 unknown；
仅当调用方能够证明输入与权威表同源时，才可显式启用保序桥接。
"""
...
from .domain_packs import DomainPackSourceError, is_external_domain_pack, resolve_domain_pack_source
...
def default_subfunction_source_path(domain: str) -> Path:
    ...
    if is_external_domain_pack(canonical):
        return resolve_domain_pack_source(canonical).subfunction_path
    return Path(__file__).resolve().parents[2] / "references" / "domain_packs" / canonical / f"{canonical}_subfunctions.json"
...
def load_domain_subfunction_authority(domain: str, path: str | Path | None = None, *, sheet_name: str | None = None) -> tuple[list[DomainSubfunctionRecord], dict[str, Any]]:
    ...
    migration/tests.  The default path is always ``references/domain_packs/<DOMAIN>/<DOMAIN>_subfunctions.json``.
    """
    strict_external = is_external_domain_pack(domain)
    try:
        source_path = Path(path) if path else default_subfunction_source_path(domain)
        source_path = source_path.resolve()
        if strict_external:
            expected = resolve_domain_pack_source(domain).subfunction_path.resolve()
            if source_path != expected:
                raise DomainPackSourceError(f"严格外部 Domain Pack 子功能表不允许混用其他来源: {source_path}; 应为 {expected}")
        if not source_path.exists():
        ...
        # Explicit spreadsheet inputs remain available to offline callers only.
        ...
        if strict_external:
            raise DomainPackSourceError(f"严格外部 Domain Pack 子功能表加载失败，不允许降级: {error}") from error
        raise
...
def match_domain_subfunctions(
    ...
    *,
    ...
    allow_source_order_bridge: bool = False,
) -> list[dict[str, Any]]:
    ...
    # 正式 JSON 运行时默认禁止按顺序猜测。只有调用方已证明输入和权威表同源时，
    # 才能显式启用兼容桥接；否则未精确命中的名称必须保持 unknown。
    if not allow_source_order_bridge:
