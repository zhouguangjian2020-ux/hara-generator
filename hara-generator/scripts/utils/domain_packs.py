...
_PACK_CACHE: dict[tuple[str, str, bool], dict[str, Any]] = {}
...
class DomainPackSourceError(RuntimeError):
    """A configured external source failed; legacy fallback is not allowed."""


@dataclass(frozen=True)
class DomainPackSource:
    canonical_domain: str
    source: str
    pack_path: Path
    subfunction_path: Path


_PT_ALIASES = {"PT", "P", "POWERTRAIN"}


def is_external_domain_pack(domain: str) -> bool:
    """Whether Domain Pack/S1/case assets use HARA_DOMAIN_PACKS_ROOT."""
    return bool(str(domain or "").strip()) and bool(
        os.environ.get("HARA_DOMAIN_PACKS_ROOT", "").strip()
    )


def is_strict_external_pt(domain: str) -> bool:
    """Backward-compatible PT-only predicate for PT's scenario contract."""
    return (
        str(domain or "").strip().upper() in _PT_ALIASES
        and is_external_domain_pack(domain)
    )


def canonical_domain_pack_code_without_external_manifest(domain: str) -> str:
    """Canonicalize aliases needed to locate an external domain directory."""
    raw = str(domain or "").strip()
    return {
        "P": "PT", "POWERTRAIN": "PT", "PT": "PT",
        "A": "AD", "ADAS": "AD", "AD": "AD",
        "INFO": "ET", "INFOTAINMENT": "ET", "ET": "ET",
        "B": "BD", "BODY": "BD", "BD": "BD",
        "CB": "CB", "CHASSIS_BRAKE": "CB",
        "CS": "CS", "CHASSIS_STEERING": "CS",
    }.get(raw.upper(), raw)


def _pack_directory() -> Path:
    """Installed manifest and non-PT assets keep their existing location."""
...
def canonical_domain_pack_code(domain: str) -> str:
    """Resolve aliases without making external mode depend on the installed manifest."""
    raw = (domain or "").strip()
    if not raw:
        return ""
    if is_external_domain_pack(raw):
        return canonical_domain_pack_code_without_external_manifest(raw)
...
def resolve_domain_pack_source(domain: str) -> DomainPackSource:
    """Resolve a coherent Pack/S1 pair from external or installed assets."""
    if is_external_domain_pack(domain):
        raw_root = os.environ["HARA_DOMAIN_PACKS_ROOT"].strip()
        try:
            root = Path(raw_root).expanduser()
            if not root.is_absolute():
                raise DomainPackSourceError("HARA_DOMAIN_PACKS_ROOT 必须是绝对路径: " + raw_root)
            root = root.resolve()
            canonical = canonical_domain_pack_code_without_external_manifest(domain)
            source = DomainPackSource(
                canonical_domain=canonical, source="workspace",
                pack_path=root / canonical / f"{canonical}.json",
                subfunction_path=root / canonical / f"{canonical}_subfunctions.json",
            )
            missing = [str(p) for p in (source.pack_path, source.subfunction_path) if not p.is_file()]
        except (OSError, ValueError, RuntimeError) as error:
            if isinstance(error, DomainPackSourceError):
                raise
            raise DomainPackSourceError(f"HARA_DOMAIN_PACKS_ROOT 无效: {raw_root}") from error
        if missing:
            raise DomainPackSourceError(
                "严格外部 Domain Pack 不允许回退安装包；缺失资产：" + ", ".join(missing)
            )
        return source
    ...
    filename = load_domain_pack_manifest().get("domain_files", {}).get(canonical)
    if not filename:
        raise ValueError(f"未配置域 '{domain}' 的 Domain Pack")
    return DomainPackSource(
        canonical_domain=canonical, source="installed",
        pack_path=_pack_directory() / filename,
        subfunction_path=_pack_directory() / canonical / f"{canonical}_subfunctions.json",
    )


def load_domain_pack(domain: str, *, validate: bool = True) -> dict[str, Any]:
    """按解析后的绝对来源加载；不同来源及校验模式不共用缓存。"""
    source = resolve_domain_pack_source(domain)
    cache_key = (source.canonical_domain, str(source.pack_path.resolve()), validate)
    if cache_key in _PACK_CACHE:
        return _PACK_CACHE[cache_key]

    try:
        value = json.loads(source.pack_path.read_text(encoding="utf-8"))
        if not isinstance(value, dict):
            raise ValueError(f"{source.canonical_domain} Domain Pack 顶层必须是对象")
        if validate:
            errors = validate_domain_pack(value, expected_domain=source.canonical_domain)
            if errors:
                raise ValueError(f"{source.canonical_domain} Domain Pack 无效：" + "; ".join(errors))
    except (OSError, ValueError) as error:
        if source.source == "workspace":
            raise DomainPackSourceError(
                f"严格外部 Domain Pack 加载失败，不允许降级: {source.pack_path}: {error}"
            ) from error
        raise ValueError(f"无法加载 {source.canonical_domain} Domain Pack: {source.pack_path}: {error}") from error

    _PACK_CACHE[cache_key] = value
    return value
            ...
            for index, rule in enumerate(scope_rules):
                ...
                if disposition == "transfer":
                    if not isinstance(target_domain, str) or not target_domain.strip():
                        ...
                        target_domain.strip().upper() in _PT_ALIASES
                        if is_strict_external_pt(domain)
                        else canonical_domain_pack_code(target_domain) == canonical_domain_pack_code(domain)
                    ):
                        # External PT only needs to reject self-transfers; it
                        # must not load installed aliases for unrelated domains.
