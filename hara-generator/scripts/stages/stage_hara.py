...
def _build_failure_profile(case_lib, data: dict, mode: str, anomaly_text: str) -> dict:
    """Keep source semantics across prepare/validate; infer text only for legacy inputs."""
    raw = data.get("failure_profile")
    if raw is not None and not isinstance(raw, dict):
        raise ValueError("failure_profile 必须是对象")
    profile = case_lib._normalize_failure_profile(raw)
    outer_mode = case_lib._normalize_failure_profile({"canonical_mode": mode})["canonical_mode"]
    if profile["canonical_mode"] and profile["canonical_mode"] != outer_mode:
        raise ValueError("failure_profile.canonical_mode 与 failure_mode 不一致")
    explicit_class = str(data.get("anomaly_class") or "").strip()
    if profile["anomaly_class"] and explicit_class and profile["anomaly_class"] != explicit_class:
        raise ValueError("failure_profile.anomaly_class 与 anomaly_class 不一致")
    return {
        "canonical_mode": outer_mode,
        "anomaly_class": profile["anomaly_class"] or explicit_class
        or case_lib.canonical_anomaly_class(anomaly_text, mode),
    }
...
def _validate_source_case_lock(domain: str, hazards: list[dict]) -> list[tuple[str, str]]:
    ...
    for index, hazard in enumerate(hazards):
        if not hazard.get("source_case_locked"):
            continue
        try:
            failure_profile = _build_failure_profile(
                case_lib, hazard, hazard.get("failure_mode", ""), hazard.get("anomaly", ""),
            )
            hazard_profile = {
                key: hazard.get(key)
...
def _validate_s4_source_traceability(
        ...
        for anomaly in entry.get("anomalies", []):
            ...
            for source_hazard in anomaly.get("hazards", []):
                ...
                expected_source[source_key] = {
                    ...
                    "failure_profile": anomaly.get("failure_profile"),
                }
    ...
    for index, hazard in enumerate(hazards):
        ...
        if hazard.get("source_case_locked"):
            ...
            if source.get("failure_profile") is not None and hazard.get("failure_profile") != source["failure_profile"]:
                issues.append(("error", f"hazard[{index}] failure_profile 与本轮 S3 不一致"))
    return issues
...
def prepare(s3_path: str, output_path: str = "s4_hara_skeleton.json",
    ...
    if is_strict_external_pt(domain):
        # Source failures must surface before empty-input and legacy branches.
        get_case_library().load_domain(domain)
        ...
        for anom in entry.get("anomalies", []):
            ...
            for hz in hazards_list:
                ...
                try:
                    ...
                    domain_pack = {}
                case_pack_controlled = is_strict_external_pt(domain) or bool(
                ...
                if not skip and matrix_prefill is None and compatible_prefill is None:
                    try:
                        ...
                        failure_profile = _build_failure_profile(case_lib, anom, failure_mode, anom_desc)
                ...
                if skip:
                    ...
                    hazard_group["failure_profile"] = deepcopy(failure_profile)
                    hazard_group["evidence_status"] = "exact_known"
                    hazard_group["agent_action"] = "locked"
