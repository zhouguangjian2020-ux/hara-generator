...
- **PT**：唯一主数据为 `references/domain_packs/PT/PT.json`。场景正文仅保存在 `risk_catalog.scenario_catalog`，其他 PT 风险结构只能保存 `scenario_id`；`references/scenarios/PT.json` 和旧总表中的 PT 分支均不得存在。
- **PT S1 权威资产（部署必需）**：`references/domain_packs/PT/PT_subfunctions.json` 必须与 `PT.json` 一起部署。它是由离线 `PT_子功能.XLSX` 提取的 PT 子功能级 HARA 判定资产；运行时只读取 JSON，不读取或依赖原始 XLSX。缺失时脚本必须显式报告资产缺失，不得把全部子功能静默交给 Agent。
...
## 工作空间 Domain Pack 案例库（可选严格外部模式）

在 Agent 运行环境、每个阶段命令启动前设置：

```bash
export HARA_DOMAIN_PACKS_ROOT=/home/gem/.aily/workspace/references/domain_packs
```

该值是 Agent 所在机器的绝对路径，包含各域 Domain Pack 子目录。所有域统一读取：
...
支持的域包括 `PT`、`CS`、`CB`、`AD`、`ET`、`BD` 及 manifest 声明的域别名。Domain Pack、
S1 子功能权威表和内嵌案例库必须来自同一个外部根目录；外部文件缺失、无效或缺少必需结构时
必须报错，不得回退安装包 Domain Pack、独立旧案例文件或其他域案例。

只有 Domain Pack、S1 子功能权威表和内嵌案例库使用 `HARA_DOMAIN_PACKS_ROOT`。通用 SEC、S2 参考、
非 PT 场景、Profile、Alias、白名单和 Excel 模板仍使用安装包的既有路径，不设置全局
`HARA_REFERENCES_ROOT`。

未设置变量时，保持安装包分域目录及 manifest 的原有行为。

同一份 S1～S5/Excel 产物链必须保持相同环境和数据版本，不得中途换库。不同任务之间切换根路径时，
Pack、案例与 PT 场景缓存会识别新的来源；同一路径文件的在线内容更新不属于自动热重载合同，
应在任务结束后更新，并启动新进程。`write` 生成的运行清单记录实际 Pack/子功能路径、SHA-256
及 `domain_asset_source`；外部模式的运行摘要跳过未使用的安装包 Domain Pack，并纳入外部资产哈希。
...
案例库使用语义匹配，不依赖项目 `func_id`。PT 的 490 条案例内嵌在 `references/domain_packs/PT/PT.json.case_library_catalog.cases`；其他域如未来启用独立 v2 案例资产，仍只能作为各自域的扩展点。
...
- **S4 场景库按域维护**：PT 场景单源位于 `references/domain_packs/PT/PT.json`；CS/CB/AD/ET/BD 场景文件位于 `references/scenarios/`。PT 不得回退到旧总表或全局 SEC 库，CS/CB 的智能分支仍在对应域文件中维护。
