...
def extract_failure_mode_sheet(ws):
    ...
    for col, header in headers.items():
        if "功能" in header and "ID" in header:
            col_func_id = col
        elif "功能" in header:  # 包含"功能"的列都视为功能名列
            col_func_name = col
    ...
    print(f"功能ID列: {col_func_id}, 功能名列: {col_func_name}")
    ...
    for i, rec in enumerate(records[:5], 1):
        print(f"  {i}. {rec['function_id']}: {rec['function_name']} - {rec['failure_modes']}")
...
def build_complete_domain_pack(failure_modes, hazop_records, hara_records):
    """构建完整Domain Pack"""
    print(f"\n{'='*60}")
    print("构建完整Domain Pack")
    print(f"{'='*60}")

    # 1. failure_mode_rules
    failure_mode_rules = {}
    for fm in failure_modes:
        failure_mode_rules[fm["function_id"]] = fm["failure_modes"]
    ...
    for rec in hara_records:
        if rec["scenario"] and rec["scenario"] not in scenarios:
            scenarios[rec["scenario"]] = f"PT_SC_{counter:02d}"
            counter += 1
    ...
    print(f"失效模式规则: {len(failure_mode_rules)} 个功能")
    ...
    domain_pack = {
        ...
        "provenance": {
            ...
            "source_files": ["ALL-HARA_FUSA-100-GPFA-H002______FUSA HARA PT DOMAIN.xlsx"]
        },
        "function_catalog": {
            "function_matchers": [],
            "semantic_roles": {},
...
def main():
    ...
    excel_path = r"E:\Git projects\hara-generator-skill\数据组\数据组5\ALL-HARA_FUSA-100-GPFA-H002______FUSA HARA PT DOMAIN.xlsx"
    ...
    # 提取各个表
    failure_modes = extract_failure_mode_sheet(wb['失效模式'])
    hazop_records = extract_hazop_sheet(wb['HAZOP 分析'])
    hara_records = extract_hara_sheet(wb['HARA分析  '])

    # 构建Domain Pack
    domain_pack = build_complete_domain_pack(failure_modes, hazop_records, hara_records)
