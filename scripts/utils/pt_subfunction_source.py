"""PT 域子功能 HARA 权威表读取与可审计匹配。

本模块只服务 PT 域。表格中的 ``HARA判定`` 是子功能 S1 的权威结论；
...
SOURCE_FILENAME = "PT_子功能.XLSX"  # offline build input only
...
def default_pt_subfunction_source_path() -> Path:
    """返回部署时使用的PT子功能JSON权威资产路径。

    PT_子功能.XLSX仅用于离线构建，不是运行时依赖。
    """
    return Path(__file__).resolve().parents[2] / "references" / "domain_packs" / RUNTIME_FILENAME
...
def _load_pt_subfunction_authority_xlsx(source_path: Path, *, sheet_name: str | None = None) -> tuple[list[PTSubfunctionRecord], dict[str, Any]]:
    """离线读取并校验原始 PT 子功能 Excel。"""
    source_path = source_path.resolve()
    if not source_path.exists():
        raise FileNotFoundError(f"PT 子功能权威表不存在: {source_path}")

    workbook = load_workbook(source_path, read_only=True, data_only=True)
    if not workbook.worksheets:
        raise ValueError("PT 子功能权威表没有工作表")
    if sheet_name:
        if sheet_name not in workbook.sheetnames:
            raise ValueError(f"PT 子功能权威表缺少工作表: {sheet_name}")
    ...
    try:
        ...
        raise ValueError("PT 子功能权威表为空") from error
    ...
    if missing:
        raise ValueError(f"PT 子功能权威表缺少列: {', '.join(missing)}")

    records: list[PTSubfunctionRecord] = []
    errors: list[str] = []
    for row_number, values in enumerate(rows, 2):
        if not any(value not in (None, "") for value in values):
            continue
        get = lambda key: values[columns[key]] if columns[key] < len(values) else None
        ...
        records.append(PTSubfunctionRecord(
    ...
    grouped: dict[tuple[str, str, str], list[PTSubfunctionRecord]] = defaultdict(list)
    ...
    if errors:
        raise ValueError("PT 子功能权威表校验失败: " + "; ".join(errors))
    summary = {
...
def _record_from_json(payload: dict[str, Any], index: int) -> PTSubfunctionRecord:
    ...
    if not isinstance(source_row, int):
        raise ValueError(f"PT子功能JSON records[{index}].source_row必须为整数")
    hara = payload.get("hara")
    if hara not in {True, False, None}:
        raise ValueError(f"PT子功能JSON records[{index}].hara必须为true/false/null")
    return PTSubfunctionRecord(
...
def _validate_runtime_records(
    records: list[PTSubfunctionRecord], *, source_path: Path, metadata: dict[str, Any]
) -> dict[str, Any]:
    ...
    if errors:
        raise ValueError("PT子功能JSON校验失败: " + "; ".join(errors))
    summary = {
        "schema_version": SOURCE_SCHEMA,
    ...
    if conflicts:
        raise ValueError("PT子功能JSON存在HARA判定冲突: " + json.dumps(conflicts, ensure_ascii=False))
    return summary


def _load_pt_subfunction_authority_json(source_path: Path) -> tuple[list[PTSubfunctionRecord], dict[str, Any]]:
    """Load the normalized PT subfunction authority JSON used at runtime."""
    try:
        payload = json.loads(source_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as error:
        raise ValueError(f"PT子功能JSON格式无效: {source_path}") from error
    if not isinstance(payload, dict):
        raise ValueError("PT子功能JSON根节点必须为对象")
    if payload.get("schema_version") != SOURCE_SCHEMA:
        raise ValueError(
            f"PT子功能JSON schema_version必须为{SOURCE_SCHEMA}: {payload.get('schema_version')!r}"
        )
    records_payload = payload.get("records")
    if not isinstance(records_payload, list):
        raise ValueError("PT子功能JSON缺少records数组")
    ...
    if len(records) != len(records_payload):
        raise ValueError("PT子功能JSON records中的每一项必须为对象")
    ...
    if declared_count is not None and declared_count != len(records):
        raise ValueError(f"PT子功能JSON record_count={declared_count!r}与实际记录数{len(records)}不一致")
    ...
    if declared_conflicts is not None and declared_conflicts != summary["conflict_count"]:
        raise ValueError("PT子功能JSON conflict_count与实际冲突数不一致")
    return records, summary


def load_pt_subfunction_authority(path: str | Path | None = None, *, sheet_name: str | None = None) -> tuple[list[PTSubfunctionRecord], dict[str, Any]]:
    ...
    """
    source_path = Path(path) if path else default_pt_subfunction_source_path()
    source_path = source_path.resolve()
    if not source_path.exists():
        raise FileNotFoundError(f"PT子功能权威JSON不存在: {source_path}")
    if source_path.suffix.lower() == ".json":
        return _load_pt_subfunction_authority_json(source_path)
    # Legacy explicit input path: never used by the deployed runtime default.
    if source_path.suffix.lower() in {".xlsx", ".xlsm", ".xltx", ".xltm"}:
        return _load_pt_subfunction_authority_xlsx(source_path, sheet_name=sheet_name)
    raise ValueError(f"PT子功能权威资产必须为JSON: {source_path}")

def _match_exact(candidates: list[PTSubfunctionRecord], feature_name: str) -> list[PTSubfunctionRecord]:
    key = normalize_feature_name(feature_name)
    return [record for record in candidates if record.feature_key == key]


def match_pt_subfunctions(
    function_name: str,
    ...
    candidates = [record for record in records if record.function_key == function_key and record.domain.upper() in {"P", "PT"}]
...
def _known_result(record: PTSubfunctionRecord, method: str) -> dict[str, Any]:
    return {
...
def _ambiguous_result(records: list[PTSubfunctionRecord], method: str) -> dict[str, Any]:
    return {
...
def validate_pt_subfunction_authority(path: str | Path | None = None) -> dict[str, Any]:
    """供测试/发布门调用的表格校验入口。"""
    records, summary = load_pt_subfunction_authority(path)
    if summary["conflict_count"]:
        raise ValueError(f"PT 子功能权威表存在 {summary['conflict_count']} 个冲突匹配对象")
    return summary
