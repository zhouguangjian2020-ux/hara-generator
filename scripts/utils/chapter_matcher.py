...
from utils.document_context import build_node_summary_map
            ...
            if (not ok or legacy_low_conf) and document_context:
                ...
                if deep.get("matched"):
                    ...
                    if deep.get("description"):
                        sub.description = deep["description"]
...
def _build_deep_context_index(document_context: dict) -> dict:
    ...
    return {
        "exact": exact,
        "alias": alias,
        "summaries": build_node_summary_map(document_context, max_chars=1200),
    }
...
def _clean_context_heading(value: str) -> str:
    return re.sub(r"^[\s.．、\-—_0-9一二三四五六七八九十]+", "", str(value or "").strip())
...
def _fill_sub(sub: SubFunction, h3: dict, h4: dict | None) -> None:
    if h4 is not None:
        ...
        description = h4["desc"] or h3["desc"] or ""
    else:
        sub.chapter = h3["label"]
        description = h3["desc"]
        if not description:
            description = next((h["desc"] for h in h3["h4s"] if h["desc"]), "")
