...
PT 域运行时权威资产位于 `references/domain_packs/PT_subfunctions.json`。原始 `PT_子功能.XLSX` 只用于离线构建该 JSON，Agent 运行时不得要求上传或读取该 XLSX。
...
- PT：`references/domain_packs/PT.json` 是唯一场景主数据；场景正文只在 `risk_catalog.scenario_catalog` 中保存。
